...
public abstract class FormControl extends Segment {
	 ...
	 *
	 * @return the <a target="_blank" href="http://www.w3.org/TR/html401/interact/forms.html#control-name">name</a> of the control, or <code>null</code> if the <code>name</code> attribute is missing.
	 */
	...
	private void verifyName() {
		...
		if (source.logger.isErrorEnabled()) source.logger.error(getElement().source.getRowColumnVector(getElement().begin).appendTo(new StringBuilder(200)).append(": compulsory \"name\" attribute of ").append(formControlType).append(" control is empty").toString());
	}
