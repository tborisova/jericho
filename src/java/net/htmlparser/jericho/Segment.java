...
public class Segment implements Comparable<Segment>, CharSequence {
	...
	public List<StartTag> getAllStartTags(String name) {
		...
		if (source.wasFullSequentialParseCalled() && isXMLTagName) {
			list.add(startTag);
			while (true) {
				startTag=checkEnclosure((StartTag)startTag.getNextTag(StartTagType.NORMAL));
				if (startTag==null) break;
				if (startTag.name.equals(name) || (startTag.name.startsWith(name) && startTag.isPartialNameSearchMatch(name))) list.add(startTag);
			}
		} else {
			do {
				list.add(startTag);
	...
	public List<StartTag> getAllStartTags(final String attributeName, final String value, final boolean valueCaseSensitive) {
		...
		if (source.wasFullSequentialParseCalled()) {
			...
			while (true) {
				startTag=checkEnclosure(startTag.getNextStartTag());
				if (startTag==null) break;
				if (startTag.hasAttribute(attributeName,value,valueCaseSensitive)) list.add(startTag);
			}
		} else {
			do {
				list.add(startTag);
	...
	public List<StartTag> getAllStartTags(final String attributeName, final Pattern valueRegexPattern) {
		...
		if (source.wasFullSequentialParseCalled()) {
			...
			while (true) {
				startTag=checkEnclosure(startTag.getNextStartTag());
				if (startTag==null) break;
				if (startTag.hasAttribute(attributeName,valueRegexPattern)) list.add(startTag);
			}
		} else {
			do {
				list.add(startTag);
	 ...
	 * <p>
	 * This is functionally equivalent to {@link #getAllStartTagsByClass(String) getAllStartTagsByClass(className)}<code>.get(0)</code>,
	 * but does not search beyond the first start tag and returns <code>null</code> if no such start tag exists.
	 *
	 * @param className  the class name (case sensitive) to search for, must not be <code>null</code>.
	 ...
	 * <p>
	 * This is functionally equivalent to {@link #getAllElementsByClass(String) getAllElementsByClass(className)}<code>.get(0)</code>,
	 * but does not search beyond the first enclosed element and returns <code>null</code> if no such element exists.
	 * <p>
